  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        if (!userSnap.exists()) {
          console.error('[Operator Guard] Usuario no encontrado en colección usuarios:', cleanEmail);
          setDenied(true);
          setLoading(false);
          return;
        }
        ...
        // 'super', 'admin' y 'operator' pueden entrar a este portal
        if (role === 'super' || role === 'admin' || role === 'operator') {
          setCurrentUser(firebaseUser);
          setDenied(false);
        } else {
          console.error(`[Operator Guard] Acceso no autorizado con rol "${role}" por ${cleanEmail}`);
          setDenied(true);
        }
      } catch (err) {
        console.error('[Operator Guard Error]', err);
        setDenied(true);
      } finally {
    ...
    return (
      <div className="min-h-screen bg-[#07110B] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-xs text-zinc-400 font-mono tracking-widest uppercase">
          {isEs ? 'Verificando credenciales de operador...' : 'Verifying operator credentials...'}
        </p>
      </div>
    );
  }
  ...
  if (denied) {
    return (
      <div className="min-h-screen bg-[#0A0A0F] text-zinc-100 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-zinc-950/80 border border-teal-900/40 rounded-3xl p-8 text-center space-y-6 shadow-2xl shadow-teal-950/50">
          ...
          <div className="space-y-2">
            ...
            <h2 className="font-serif text-2xl font-bold text-white">
              {isEs ? 'Acceso Operativo Restringido' : 'Restricted Operator Portal'}
            </h2>
            <p className="text-xs text-zinc-400 leading-relaxed">
              {isEs
                ? 'Este portal está destinado exclusivamente a Operadores Logísticos y Guías de Vermilion Routes.'
                : 'This portal is strictly for authorized Vermilion Routes Logistical Concierges & Guides.'}
            </p>
            {userRole && (
              <p className="text-[11px] text-zinc-500 font-mono mt-1">
                {isEs ? 'Tu rol actual:' : 'Your current role:'} <span className="text-teal-400 font-bold uppercase">{userRole}</span>
              </p>
            )}
          </div>
