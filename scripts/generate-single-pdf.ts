...
const outputFolder = path.resolve('C:/Users/pablo/Desktop/vermilion-pdfs');
...
function renderTourIntoDoc(doc: any, tour: Tour, isFirstTour: boolean = false, logoBase64: string | null = null) {
  ...
  const destination = getLocalized(tour.destination, 'es') || 'Ecuador';
  const duration = getLocalized(tour.duration, 'es') || (isDailyTour ? '1 DÍA (FULL DAY)' : '7 DÍAS');
  const category = getLocalized(tour.category, 'es') || (isDailyTour ? 'Excursión Full-Day' : 'Expedición Privada');
  const description = getLocalized(tour.description, 'es');
  ...
  if (itinerary.length > 0) {
    ...
    const hasAndes = itinerary.some(d => {
      const tText = getLocalized(d.title, 'es').toLowerCase();
      const dText = getLocalized(d.description, 'es').toLowerCase();
      return /quito|baños|banos|amazon|tena|puyo|quilotoa|cotopaxi|cuenca|otavalo/i.test(`${tText} ${dText}`);
    });
    const hasGalapagos = itinerary.some(d => isGalapagosRegion(getLocalized(d.title, 'es'), getLocalized(d.description, 'es')));
    const isMultiRegionCombo = hasAndes && hasGalapagos;

    for (let i = 0; i < itinerary.length; i++) {
      const dayItem = itinerary[i];
      const dayTitle = getLocalized(dayItem.title, 'es');
      const rawDayDesc = getLocalized(dayItem.description, 'es');
      const dayMeals = dayItem.meals ? getLocalized(dayItem.meals, 'es') : '';
      const dayAcc = dayItem.accommodation ? getLocalized(dayItem.accommodation, 'es') : '';
      const dayTrans = dayItem.transportation ? getLocalized(dayItem.transportation, 'es') : '';
      const dayAct = dayItem.activity ? getLocalized(dayItem.activity, 'es') : '';
  ...
  if (inclusions.length > 0 || exclusions.length > 0) {
    ...
    inclusions.forEach((inc) => {
      const incText = getLocalized(inc, 'es');
    ...
    if (exclusions.length > 0) {
      ...
      exclusions.forEach((exc) => {
        const excText = getLocalized(exc, 'es');
...
async function main() {
  ...
  console.log(`Compiling all ${allTours.length} tours into a SINGLE master PDF...`);
  ...
  for (let i = 0; i < allTours.length; i++) {
    ...
    const isFirst = (i === 0);
    const tourTitle = getLocalized(tour.title, 'es');
    console.log(`Adding Tour ${i + 1}/${allTours.length}: ${tourTitle}`);
    renderTourIntoDoc(doc, tour, isFirst, logoBase64);
  }

  // Apply footers and dynamic page numbers to the whole catalog
  const totalPages = doc.getNumberOfPages();
  const pageWidth = 210;
  const pageHeight = 297;
  const marginX = 14;
  const marginRight = 14;
  const footerText = "Vermilion South American Routes • Operador Turístico Certificado • Quito, Ecuador";
...
main().catch(err => {
  console.error('Fatal error generating master PDF:', err);
  process.exit(1);
});
