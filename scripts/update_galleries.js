...
tours.forEach(tour => {
  ...
  let gallery = [tour.imageUrl]; // Main image is always first in gallery
  ...
  // Shuffle the remaining gallery (excluding the first one) to give some variety
  const head = gallery[0];
  const tail = gallery.slice(1).sort(() => Math.random() - 0.5).slice(0, 4); // Keep up to 5 images total
